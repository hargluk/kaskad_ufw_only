...
| **Удаление одного правила** | Точечное удаление из активных цепочек `iptables` | Удаление строк из `before.rules` → `ufw reload` |
| **Полная очистка** | Сброс **всех** цепочек `iptables` (чужие правила тоже удаляются) | Удаление **только своих строк** из `before.rules`; стандартные правила UFW сохраняются |
| **Безопасность для сторонних сервисов** | Низкая (можно нарушить работу Docker и др.) | Высокая (изменения только в своих правилах) |
...
6. Сбросить ВСЕ правила (только добавленные скриптом)
0. Выход
